...
function MonitorActor(aConnection,tabActor) {
  this.conn = aConnection;
  this._updates = [];
  this._started = false;
  globalWindow = tabActor.window;
}
...
let WhateverAgent = {
  ...
  start: function() {
    WhateverAgent.update();
    globalWindow.addEventListener("devicelight", e => {
      WhateverAgent._packet.time = Date.now();
      WhateverAgent._packet.value = e.value;
    }, true);
  },

  update: function() {
    if (WhateverAgent._packet.value) {
      Services.obs.notifyObservers(null, "devtools-monitor-update", JSON.stringify(WhateverAgent._packet));
    }
    WhateverAgent._timeout = setTimeout(WhateverAgent.update, 300);
  },
...
//MonitorActor.prototype._addAgent(WhateverAgent);
